...
(function ($) {
  ...
  $.when(
    ...
    (data_set["ccc-posts_per_page"] = post_area_elm.data(
      "ccc_my_favorite-posts_per_page"
    )),
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        $("#" + count_post)
          .children(".number")
          .text(post_count_val);
        var found_posts_count_val = $(".ccc-favorite-post-count")
          .find(".num")
          .text();
        $("#" + count_post)
          .children(".found_posts")
          .hide();
        if (Number(post_count_val) < Number(found_posts_count_val)) {
          $("#" + more_trigger).fadeIn();
          $("#" + count_post)
            .children(".found_posts")
            .text(found_posts_count_val)
            .fadeIn();
        } else {
          $("#" + more_trigger).hide();
        } //endif
  ...
  function my_favorite_delete(this_elm) {
    if (this_elm.parent().data("ccc_favorite-delete_all") == true) {
      var my_promise = $.when($("." + list_post).fadeOut());
    } else {
      var my_promise = $.when(this_elm.closest("." + list_post).fadeOut());
    } //endif
